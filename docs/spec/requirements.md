...
| CH-12 | An application MAY be assigned to **more than one channel at once** (e.g. mic → Voice *and* Stream); every assigned channel hears it, the assignment persists like CH-4. | owner 2026-09-15 | 📝 |
...
| UX-10 | Every running application row MUST show the app's own icon and whether it is currently producing sound; apps on no channel MUST be visible as such so they can be picked up. | owner 2026-09-15 | 📝 |
| UX-11 | Assigning an app to a channel MUST work by drag-and-drop onto the channel row (in addition to the picker); dropping onto several rows accumulates the assignment (CH-12), dropping somewhere else replaces it. | owner 2026-09-15 | 📝 |
| UX-12 | Every entity (channel *and* mix) MUST have a listen button: press-and-hold routes exactly that entity to the main output, release restores the previous routing (solo audition). Works on a device with one built-in speaker as well as phones. | owner 2026-09-15 | 📝 |
