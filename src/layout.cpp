...
QString Layout::toPipewireConf() const {
    ...
    const QString loop = QStringLiteral(
        ...
        "playback.props = { node.name = %4 media.name = %4 node.target = %5 node.dont-reconnect = true node.dont-fallback = true %6} } }\n");
    for (const auto &c : channels)
        for (const auto &m : mixes) {
            const QString cell = Names::cellNode(c.slug, m.slug);
            out += loop.arg(q(c.name + QStringLiteral(" → ") + m.name), q(cell + QStringLiteral(".in")), q(Names::channelNode(c.slug)), q(cell), q(Names::mixNode(m.slug)), QString());
        }
    for (const auto &m : mixes) {
        ...
        out += loop.arg(q(QStringLiteral("Mix: ") + m.name + QStringLiteral(" → output")), q(outNode + QStringLiteral(".in")), q(Names::mixNode(m.slug)), q(outNode),
                        q(m.outputDevice.isEmpty() ? QStringLiteral("kmixdeck.unrouted") : m.outputDevice), QString());
